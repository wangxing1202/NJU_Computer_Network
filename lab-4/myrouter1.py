...
class ArpTable(object):
    ...
    def get(self,ipaddr):
        return self.arptable.get(ipaddr)

    def update(self,ipaddr,ethaddr):
        self.arptable[ipaddr]=ethaddr
...
class ForwardingTable(object):
    ...
    def __init__(self, interfaces:list)->None:
        for interface in interfaces:
            ...
            self.forwardingtable.append([netaddr,interface.netmask,IPv4Address('0.0.0.0'),interface.name])
        with open('forwarding_table.txt') as lines:
            for line in lines:
                string=line.split(' ')
                netaddr=IPv4Network(string[0]+'/'+string[1])
                self.forwardingtable.append([netaddr,IPv4Address(string[1]),IPv4Address(string[2]),string[3].rstrip('\n')])
    
    def query(self,ipaddr:IPv4Address):
        prefix_len=0
        interface=None
        next_addr=None
        for netaddr,netmask,next_addr_1,interface_1 in self.forwardingtable:
...
class IpQueue(object):
    ...
    def get(self,ipaddr):
        return self.ipqueue.get(ipaddr)
    
    def push(self,ipaddr,info:list):
        self.ipqueue[ipaddr]=[info]

    def append(self,ipaddr,info:list):
        self.ipqueue[ipaddr].append(info)

    def delete(self,ipaddr):
        del self.ipqueue[ipaddr]
    
    def get_info(self,ipaddr):
        return list(self.ipqueue[ipaddr])

class ArpQueue(object):
    arpqueue={}

    def get(self,ipaddr):
        return self.arpqueue.get(ipaddr)

    def update(self,ipaddr,info:list):
        self.arpqueue[ipaddr]=info

    def delete(self,ipaddr):
        del self.arpqueue[ipaddr]

    def keys(self):
        return self.arpqueue.keys()     
